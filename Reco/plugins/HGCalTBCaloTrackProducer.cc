...
HGCalTBCaloTrackProducer::HGCalTBCaloTrackProducer(const edm::ParameterSet& cfg) : 
  ...
  _minEnergy(cfg.getUntrackedParameter<double>("minEnergy",0.25)),
  _maxEnergy(cfg.getUntrackedParameter<double>("maxEnergy",4))
{
...
void HGCalTBCaloTrackProducer::produce(edm::Event& event, const edm::EventSetup& iSetup)
{
  ...
  for( auto cluster : *clusters ){
    if( cluster.energy()<_minEnergy || cluster.energy()>_maxEnergy || cluster.layer()>5 ) 
      continue;
    ...
    for( std::vector< std::pair<DetId,float> >::const_iterator it=cluster.hitsAndFractions().begin(); it!=cluster.hitsAndFractions().end(); ++it ){
      ...
      hitenergies.push_back( (*it).second*cluster.energy() );
    }
  }
  if( (int)touchedLayers.size()>_minTouchedLayers ){
    reco::LeastSquare<reco::HGCalTBClusterCollection> wls;
    ...
    if( success!=false ){
      ...
      std::cout << "Find a track : \n";
      for( size_t ii=0; ii<detIds.size(); ii++ ){
      	std::cout << detIds[ii] << "\t" << hitenergies[ii] << std::endl;
      }
    }
  }
