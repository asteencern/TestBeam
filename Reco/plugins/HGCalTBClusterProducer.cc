...
HGCalTBClusterProducer::HGCalTBClusterProducer(const edm::ParameterSet& cfg) : 
  m_electronicMap(cfg.getUntrackedParameter<std::string>("ElectronicMapFile",std::string("HGCal/CondObjects/data/map_CERN_8Layers_Sept2016.txt"))),
...
void HGCalTBClusterProducer::produce(edm::Event& event, const edm::EventSetup& iSetup)
{
  ...
  for(auto hit : *rechits ){
    if( hitmap.find( hit.id().layer() )!=hitmap.end() && hit.energy()>m_minEnergy )
